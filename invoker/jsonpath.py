...
class DatumInContext(object):
    ...
    def in_context(self, context, path):
        ...
        if self.context:
            return DatumInContext(value=self.value, path=self.path, context=context.in_context(path=path,
                                                                                               context=context))
        else:
...
class Descendants(JSONPath):
    ...
    def delete(self, data):
        ...
        def delete_recursively(data):
            ...
            if isinstance(data, list):
                for i in range(0, len(data)):
                    delete_recursively(data[i])

            elif isinstance(data, dict):
                for field in data.keys():
                    delete_recursively(data[field])

        for submatch in left_matches:
            delete_recursively(submatch.value)
